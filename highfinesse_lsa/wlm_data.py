...
from ctypes import CFUNCTYPE, c_double, c_long, c_ssize_t, windll
...
class MeasurementType(Enum):
    ...
    exposureTime1 = cmiExposureValue1

    #: Exposure time 2, in milliseconds. Updated on manual changes as well as
    #: due to automatic adjustment being active.
    exposureTime2 = cmiExposureValue2


_DOUBLE_MEASUREMENT_TYPES = {
    MeasurementType.deviceTemperature,
...
class LSA:
    ...
    def add_callback(self, cb: Callable[[MeasurementType, Union[int, float]], None]) -> None:
        """
    ...
    def remove_callback(self, cb: Callable[[MeasurementType, Union[int, float]], None]) -> None:
        """Unregister a previously added measurement result callback."""
        with self._result_callbacks_lock:
            self._result_callbacks.remove(cb)

    def _callback_ex(self, ver: c_long, mode: c_long, intval: c_long, dblval: c_double, res1: c_long) -> None:
